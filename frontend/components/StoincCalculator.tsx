...
export function StoincCalculator() {
    ...
    const [solReward, setSolReward] = useState(0);
    ...
    useEffect(() => {
        ...
        // Note: Using aggregates. If pNodes=3, we use 3 * totalStorage...?
        // User formula: pNodes × storageSpace × performanceScore × stake
        // Interpretation: Is 'storageSpace' PER NODE or TOTAL?
        // Logic check: Ideally it means (Sum of individual unboosted credits) OR (pNodes * Total).
        // Standard staking formula usually sums individual contributions.
        // BUT the user strictly said: "Nhập số lượng node... tính lại theo công thức CHUẨN... storageCredits = pNodes × storageSpace × performanceScore × stake"
        // If 'storageSpace' is TOTAL storage capacity, then the formula `pNodes * TotalStorage` scales quadratically.
        // We will follow the formula literally as requested for 'storageSpace' = Total capacity.
        ...
        const solPerEpoch = share * totalFees * pNodeShare;
        setSolReward(solPerEpoch);

        const monthlyXand = 10000 * pNodes; // 10k XAND per pNode
        setFixedXandReward(monthlyXand);

        // Financials
        const dailySolIncome = solPerEpoch * epochsPerDay * solPrice;
        const dailyFixedIncome = (monthlyXand / 30) * xandPrice;
        const totalDaily = dailySolIncome + dailyFixedIncome;
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="p-4 rounded-xl bg-slate-950/40 border border-slate-800/50">
                            ...
                            <div className="mt-2 flex justify-between text-xs text-slate-500">
                                <span>SOL: ${(solReward * epochsPerDay * solPrice).toFixed(2)}</span>
