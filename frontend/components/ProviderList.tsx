                ...
                {providers.slice(0, 5).map((provider, index) => (
                    <div key={provider.provider_id} className="flex items-center justify-between border-b border-slate-800/50 pb-3 last:border-0 last:pb-0">
                        <div className="flex items-center gap-3">
                            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-slate-200 dark:bg-slate-800 text-xs font-medium text-slate-600 dark:text-slate-600 dark:text-slate-400">
                                {index + 1}
                            </span>
                            <div>
                                <p className="font-medium text-slate-200">{provider.provider_name}</p>
                                <p className="text-xs text-slate-600 dark:text-slate-600 dark:text-slate-500">{provider.nodes.total_count} nodes</p>
                            </div>
                        </div>
                        <div className="text-right">
                            ...
                            <p className="text-xs text-slate-600 dark:text-slate-600 dark:text-slate-500">
